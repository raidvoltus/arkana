...
def sanitize_log_datetime_column():
    ...
    try:
        ...
        if "tanggal" not in df.columns:
            logging.error("Kolom 'tanggal' tidak ditemukan di prediksi_log.csv")
            return

        # Coba parse kolom tanggal
        df['tanggal'] = pd.to_datetime(df['tanggal'], errors='coerce')

        # Buang baris yang tanggalnya gagal di-parse
        before_rows = len(df)
        df = df.dropna(subset=['tanggal'])
        after_rows = len(df)

        if after_rows < before_rows:
            logging.warning(f"{before_rows - after_rows} baris dibuang karena format tanggal tidak valid.")

        # Simpan ulang file
        df.to_csv(log_path, index=False)
        logging.info("Sanitasi kolom tanggal selesai.")
        
    except Exception as e:
        logging.error(f"Error saat membersihkan kolom tanggal: {e}")
