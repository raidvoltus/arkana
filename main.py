...
def sanitize_log_datetime_column():
    log_path = "prediksi_log.csv"
    ...
    try:
        ...
        if "datetime" not in df.columns:
            logging.error("Kolom 'datetime' tidak ditemukan di prediksi_log.csv")
            return
        ...
        if after < before:
            logging.warning(f"{before - after} baris dibuang karena datetime tidak valid.")

        df.to_csv(log_path, index=False)
        logging.info("Sanitasi kolom datetime selesai.")

    except Exception as e:
        logging.error(f"Error saat membersihkan kolom datetime: {e}")

# === Header Validator ===
def check_and_update_csv_header():
    log_path = "prediksi_log.csv"
    expected_cols = ["ticker", "datetime", "actual_close", "predicted_high", "predicted_low", "model"]
...
# === Prediction Logger ===
def log_prediction(ticker: str, datetime_str: str, pred_high: float, pred_low: float, actual_close: float, model_name: str = "LightGBM-v1"):
    file_exists = os.path.exists("prediksi_log.csv")
    with open("prediksi_log.csv", "a") as f:
        if not file_exists:
            f.write("ticker,datetime,actual_close,predicted_high,predicted_low,model\n")
        f.write(f"{ticker},{datetime_str},{actual_close},{pred_high},{pred_low},{model_name}\n")

    logging.info(
        f"[{model_name}] Prediksi {ticker} @ {datetime_str} | Actual: {actual_close:.2f} | "
    ...
    try:
        ...
        for ticker in df['ticker'].unique():
            ...
            if mae > 5.0:
                ...
                model = LGBMRegressor()
                features = data_ticker[['actual_close']].values
...
# === Eksekusi saat import ===
clean_log_file()
sanitize_log_datetime_column()
